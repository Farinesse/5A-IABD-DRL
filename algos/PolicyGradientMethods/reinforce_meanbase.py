...
class REINFORCEWithBaseline:
    def __init__(self, state_dim, action_dim, alpha=0.0003, alpha_value=0.0003, gamma=0.99):
        ...
        self.value = self._build_value()  # Nouveau réseau pour la baseline

        self.policy_optimizer = tf.keras.optimizers.Adam(learning_rate=self.alpha)
        self.value_optimizer = tf.keras.optimizers.Adam(learning_rate=self.alpha_value)
        self.reward_buffer = []
    ...
    def _build_value(self):
        model = keras.Sequential([
            keras.layers.Dense(128, activation='relu', input_dim=self.state_dim),
            keras.layers.Dense(128, activation='relu'),
            keras.layers.Dense(1)  # Sortie unique: estimation de la valeur d'état
        ])
        return model
    ...
    def train_episode(self, env):
        ...
        # Entraînement du réseau de valeur (baseline)
        with tf.GradientTape() as value_tape:
            value_predictions = self.value(states)
            value_loss = tf.reduce_mean(tf.square(returns - tf.squeeze(value_predictions)))
        ...
        # Calcul de l'avantage (returns - baseline)
        advantages = returns - tf.squeeze(value_predictions)

        # Entraînement du réseau de politique avec l'avantage
        with tf.GradientTape() as policy_tape:
            ...
            # Utilisation de l'avantage au lieu du return brut
            basic_loss = -tf.reduce_mean(log_probs * advantages)

            # Ajout de l'entropie
            probs = tf.nn.softmax(logits)
            entropy = -tf.reduce_mean(tf.reduce_sum(probs * tf.math.log(probs + 1e-10), axis=1))
            policy_loss = basic_loss - 0.01 * entropy
        ...
        return sum(rewards), policy_loss.numpy(), value_loss.numpy()
    ...
    def train(self, env, episodes=5000):
        ...
        for episode in tqdm(range(episodes), desc="Training Episodes"):
            env.reset()
            total_reward, policy_loss, value_loss = self.train_episode(env)
        ...
        self.save_model('reinforce_baseline_model_Farkel.h5')
        return history

    def save_model(self, filepath):
        # Sauvegarder les deux réseaux séparément
        self.policy.save(filepath.replace('.h5', '_policy.h5'))
        self.value.save(filepath.replace('.h5', '_value.h5'))
